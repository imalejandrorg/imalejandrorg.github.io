...
ggplot(data.frame(x = c(-4, 4)), aes(x = x)) +
  ...
  xlab("") +
  scale_y_continuous(breaks = NULL) +
  scale_x_discrete(limit = c(-1.96, 1.96), labels = c("-1.96" = parse(text = TeX("$-\\t_{0.025}$")), "1.96" = parse(text = TeX("$\\t_{0.025}$")))) +
  theme_minimal_hgrid()
```
...
Una forma compacta de escribir el intervalo de confianza es con el límite inferior y superior separados por una coma y entre parentésis ($31.4, 34.2$). Con esta información sabemos entonces que $31.4cm^2 > \mu > 34.2cm^2$, nuestra media poblacional $\mu$ se encuentra, el 95% de las veces, en este intervalo.