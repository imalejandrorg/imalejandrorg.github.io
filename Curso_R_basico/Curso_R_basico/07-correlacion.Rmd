...
La siguiente gráfica muestra la relación entre las dos variables.
...
Y de nuevo, los valores críticos se calculan con los grados de libertad, $df = n - 2$. En `R` este proceso ya lo hemos realizado con la función `summary()` aplicada a nuestro modelo. El valor $0$ en la fórmula \@ref(eq:b1test) nos recuerda que estamos comparando el valor esperado bajo la $H_0$ con el valor obtenido.

```{r}
summary(Regresion)
```

En este caso podemos ver que el valor $t$ ya está dado en el apartado de los coeficientes ($t_s = 7.546$), junto con nuestro valor-*p* denotado por el título `Pr(>|t|)`, que en este caso corresponde a $valor-p = 0.000132$, por lo tanto rechazamos la $H_0$ ya que $valor-p < 0.05$.
...
Son bastante útiles para detectar anomalías en los datos (como algún patrón distinto al patrón lineal). Estos gráficos, conocidos como **gráficos de residuales** es lo que queda después de que removemos la tendencia lineal, por lo que esperamos que sean gráficos sin un patrón conciso. Por ejemplo, la figura \@ref(fig:resplot)

```{r resplot, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.align="center", fig.width=5, fig.height=5, fig.cap="Gráfico de residuales mostrando la ausencia de curvatura o patrón para los datos de serpientes."}
...
```{r resplot2, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.align='center', fig.cap="Gráfico de residuales comparados con la valores Z teóricos para mostrar la normalidad.", fig.width=5, fig.height=5}
...
Las fórmulas para el intevalo de confianza (Fórmula \@eq:ICreg) y el intervalo de la predicción (Fórmula \@eq:ICreg) son:
...
```{r confypred, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.align='center', fig.height=5, fig.width=5, fig.cap="Intervalo de confianza al 95% (gris) e intervalo de la predicción al 95% (línea azul punteada) para los datos de serpientes."}
...
Como podemos ver en la figura \@ref(fig:confypred), el intervalo de confianza es muchísimo más preciso que el intervalo de predicción, como se mencionó anteriormente. Para obtener los intervalos de confianza y de predicción en `R` podemos utilizar las siguientes funciones basadas en nuestro modelo lineal.
