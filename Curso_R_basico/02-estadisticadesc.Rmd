...
A diferencia de los estadísticos de tendencia central, los estadísticos de dispresión se basan en cómo se distribuyen los datos y qué tan esparcidos están. Algunas medidas de dispersión son:
...
Para obtener la desviación estándar en R lo único que necesitamos es utilizar la fución `sd()`. 
```{r}
...
Para facilitar la lectura vamos a transformar las frecuencias relativas a porcentajes multiplicando por 100 y utilizando la función `round()` para redondear los valores a 2 digitos.
```{r}
round((prop.table(Frecab) * 100), 2)
```
Podemos ver que el 55.56% de las aves son de habitos arbustivos mientras que el 44.44% prefieren los bosques.
...
Y si quisieramos obtener el porcentaje, de nuevo podemos utilizar la función `mutate` y multiplicando la varaible `Frecuencia_relativa` por 100%.
```{r}
...
En este caso, eliminamos los datos de los sépalos y nos quedamos con los datos solamente de los pétalos. 