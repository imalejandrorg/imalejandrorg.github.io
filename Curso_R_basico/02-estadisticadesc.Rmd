...
* **Media:** Mide el promedio del valor de nuestros datos. Se calcula como la suma de las observaciones dividida entre el número de observaciones.
...
### Media en R
El cálculo de la media en R para un conjunto de datos es relativamente sencillo. Para ello implementamos la función *mean()*.
```{r}
library(vegan)
data("iris")
...
Para el calculo de estas cercas se implementan las siguientes fórmulas:
$$
Cerca_i = Q_1 - 1.5 * IQR \\
Cerca_s = Q_3 + 1.5 * IQR
$$
