...
## Exportar datos
En algunas ocasiones queremos trabajar con otro software o queremos exportar nuestra tabla modificada a Excel. Para hacer esto, utilizamos la función *write*, por ejemplo.
```{r}
#Filtramos nuestro archivo para tener solo las abundancias.
Abun <- Aves[,4:35]

#Exportamos nuestro archivo en formato .csv
write.table(Abun, file = "Abundancias.csv", sep = ",", row.names = FALSE, col.names = FALSE)
```
...
```{r}
write.table(Abun, file = "O:/Documentos/R_Basico/Lección_1/Abundancias.csv", sep = ",", row.names = FALSE, col.names = FALSE)
```
...
## Pedir ayuda
En caso de que no sepamos que argumentos van en alguna función, podemos pedir ayuda en la consola utilizando el símbolo de interrogación ?.
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
?specaccum
```
