...
Es muy importante que nos sintamos cómodos al momento de trabajar en RStudio, sobre todo cuando son jornadas largas las que vamos a pasar programando. Personalmente, recomiendo colores oscuros que no cansen tanto la vista. Por eso mismo elegí el tema `Material`.
...
Aunque `R` normalmente no tiene problemas al leer archivos con espacio, se recomienda **no dejar espacios** en los nombre de nuestros proyectos. En su lugar, podemos utilizar guión bajo (_), el símbolo de menos (-) o un punto (.).
...
Para saber cuál es la carpeta de nuestro directorio de trabajo, podemos teclear la función `getwd()`, que nos dará la dirección que tenemos establecida como directorio de trabajo.

```{r}
getwd()
```

Para cambiar el directorio de trabajo, simplemente tecleamos en la consola la función `setwd()` con la dirección de la carpeta .
...
- **Vector:** Es uno de los elementos básicos de R. Existen distintos de vectores, los cuales pueden ser: lógicos, integers, caracteres y complejos. 
...
Sin embargo, si queremos almacenar este resultado necesitamos nombrar a una variable con este resultado. Por ejemplo, una variable llamada *suma*. Esta variable será un vector, específicamente un **integer,** que consisten de números con los que podemos realizar operaciones.
...
Estaríamos creando un caracter, no un integer... **mucho cuidado con las comillas.**
...
Para los análisis de datos de naturaleza biológica normalmente se trabaja con matrices de datos con múltiples caracteres tanto categóricos como cuantitativos, que en `R` corresponde a un `data frame`. Por ejemplo, los datos de una matriz llamada `dune` del paquete `vegan` corresponden a una matriz de datos.
...
Una de las herramientas más útiles de `R` es la instalación de paquetes. No tenemos la necesidad de escribir nuestras propias funciones cada que queramos realizar algún análisis. En su lugar, bajamos e instalamos paquetes que nos sirven para ciertas funciones.
...
```{r}
library(vegan) #Carga nuestro paquete vegan a nuestra sesión de trabajo.
```
