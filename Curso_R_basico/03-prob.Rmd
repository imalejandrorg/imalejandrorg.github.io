...
data.frame(mutantes = 0:5, prob = dbinom(x = 0:5, size = 5, prob = 0.37)) %>%
  mutate(Mutantes = ifelse(mutantes == 2, "2", "otros")) %>%
ggplot(aes(x = factor(mutantes), y = prob, width = 0.7)) +
  geom_col(colour = "black", alpha = 0) +
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.align='center', fig.cap="Distribución binomial de personas con Rh ++."}

data.frame(rh = 0:6, prob = dbinom(x = 0:6, size = 6, prob = 0.85)) %>%
  mutate(Rh = ifelse(rh == 2, "2", "otros")) %>%
ggplot(aes(x = factor(rh), y = prob, width = 0.7)) +
  geom_col(colour = "black", alpha = 0) +
  geom_text(aes(label = round(prob, 2), y = prob + 0.01), position = position_dodge(0.9), size = 3, vjust = 0) +
