...
$$
P(CR \cap OA) = P(CR) * P(OA | CR) = 70/1770*20/70 = 20/1770
$$

8.  Para dos eventos cualesquiera, $A_1$ y $A_2$:

$$
P(A_1) = P(A_2) * P(A_1 | A_2) + P(A_2^C) * P(A_1 | A_2^C )
$$ 
Donde $A_2^C$ es el complemento de $A_2$, es decir $1 - A_2$.
...
- Que exista una variable aleatoria numérica $x$, y que sus valores estén asociados con probabilidades correspondientes.
...
La probabilidad correspondiente para una variable continua equivale al área debajo de la curva de densidad entre dos puntos. Si quisiéramos saber el valor de un punto exacto obtendríamos, de manera paradójica 0 ya que lo que equivale a la probabilidad es el área bajo la curva. Algunos de los parámetros de una distribución de probabilidad son:
...
Ahora bien, la **función de masa de probabilidad (PMF)** o fórmula de la distribución binomial se indica en la fórmula \@ref(eq:binom).
...
ggplot(mutantes, aes(x = factor(n), y = prob, width = 0.7, fill = colour)) +
...
ggplot(rh, aes(x = factor(rh), y = prob, width = 0.7, fill = colour)) +
 geom_bar(stat = "identity", colour = "black", show.legend = F) +
  geom_text(aes(label = round(prob, 2), y = prob + 0.01), position = position_dodge(0.9), size = 3, vjust = 0) +
...
>**Ejemplo:** En *Escherichia coli*, una célula de cada $10^9$ muta de ser sensible a la estreptomicina a ser resistente a ella. Si observamos $2 \times 10^9$ células, ¿Cuál es la probabilidad de que ninguna mute? ¿Cuál es la probabilidad de que al menos una mute?
...
ggplot(Pois, aes(x = factor(muts), y = prob, width = 0.7, fill = colour)) +
  geom_bar(stat = "identity", colour = "black", show.legend = F) +
  geom_text(aes(label = round(prob, 2), y = prob + 0.01), position = position_dodge(0.9), size = 3, vjust = 0) +
