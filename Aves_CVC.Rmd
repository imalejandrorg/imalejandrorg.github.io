...
Datos <- readr::read_csv(
  "https://raw.githubusercontent.com/imalejandrorg/R_para_biologos/master/Bases_de_datos/Base%20de%20Datos%20Aves.csv") %>% data.frame()
...
kable(Rangos, caption = "Rangos de abundancia") %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "hover", "condensed"), full_width = FALSE) %>% scroll_box(width = "910px")
```
...
Rangos.sitios <- rankabuncomp(Aves, y = Sitios, factor = "Veg", scale = "abundance",
                              specnames = c(1:2), legend = FALSE)
```
